...
RUN dnf update -y && dnf install -y git python rpm-ostree ostree-grub2 ostree zlib-devel libuuid-devel libattr-devel libblkid-devel libselinux-devel libudev-devel parted lsscsi ksh openssl-devel elfutils-libelf-devel libtirpc-devel kernel-devel-$(uname -r) fedora-packager fedora-review libffi-devel python3-devel; dnf groupinstall "C Development Tools and Libraries" -y; dnf clean all
